 ...
 * CustomerController.java Version: 0.1.0 Turns: Turn 1 Author: Bobwares (bobwares@outlook.com)
 * Date: 2025-10-29T19:49:40Z Exports: CustomerController Description: REST controller exposing CRUD
 * operations for customers with OpenAPI documentation and validation.
 */
...
public class CustomerController {
  ...
  public ResponseEntity<CustomerDto.Response> create(
    ...
    return ResponseEntity.created(location(response.getId())).body(response);
  }
      ...
      value = {
        @ApiResponse(
            ...
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = CustomerDto.Response.class, type = "array")))
      })
  @GetMapping
  ...
  private URI location(UUID id) {
    return URI.create("/api/customers/" + id);
  }
}
